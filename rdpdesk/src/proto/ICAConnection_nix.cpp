...
// Version:     0.0
// Purpose:
// Time-stamp:  "2010-12-07 20:05:16"
...
ICAConnection::ICAConnection(MainFrame *m_frame,Options_HashMap options ,wxWindow *parent, wxWindowID id,
			 const wxPoint& pos,
			 const wxSize& size,
			 long style,
			 const wxString& name)
...
//	:wxScrolledWindow (parent, id, pos, size, 0, name)
{
	if (!wxFileExists(wxT("/usr/bin/wfica")))
	{
		wxMessageBox(_("wfica (ICA client for Linux) not found.\nPlease install ICA Citrix receiver (www.citrix.com)"),_("Error"),wxICON_ERROR);
		return;
	}
...
void ICAConnection::FullScreen(BOOL_L bRestore)
{
   if (bRestore)
   {
...
void ICAConnection::SendKey(BOOL_L cad)
{
...
void ICAConnection::GrabAll(BOOL_L state)
{
	if (state)
	{
		EnableFocus(FALSE);
...
void ICAConnection::EnableFocus(BOOL_L state)
{
	if (state)
	{
...
*/
	if (!conn->bClosing)
	{
		...
		conn->main_frame->connectionStatusUpdate(evt);
	}
...
//		int connection_width = 0;
   ...
   if (!MakeIniFile())
   {
      wxMessageBox(_("Error: create *.ini connection files failed"),_("Error"),wxICON_ERROR);
      return false;
   }
   ...
   if (!ica_window)
   {
      wxMessageBox(_("ICA Connection failed.\nClosing..."),_("Error"),wxICON_ERROR);
      Disconnect();
      return false;
   }
...
bool ICAConnection::request_close(BOOL_L bDetach)
{
   if (sock_id)
   {
      EnableFocus(FALSE);
