...
ToplevelInitBeforeRealization()
{
  ...
  //printf ("appData.title = %s \n", appData.title);
  XtVaSetValues(toplevel, XtNtitle, appData.title, XtNiconName, appData.title, NULL);
  ...
  /* if (appData.window_id != 0) */
  /* { */
  /*    if (XGetWindowAttributes(dpy, appData.window_id, &window_attributes)) */
  /*    { */
  /* 	printf ("%d\n", appData.window_id); */
  /* 	printf ("XtWindow(toplevel) = %d \n", XtWindow(toplevel)); */
	
  /*    	/\* XReparentWindow(dpy, XtWindow(toplevel), appData.window_id, 0,0); *\/ */
  /*    } */
  /* } */
